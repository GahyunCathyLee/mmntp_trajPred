...
def MMnTP_training(p, data_tuple, label_tuple, model, dataset, loss_func_tuple, device):
    ...
    # 1. label_tuple 타입 안전 처리
    if isinstance(label_tuple, (tuple, list)):
        man_data = label_tuple[0]
    else:
        man_data = label_tuple

    # 1차원일 경우 길이에 맞춰 복제 (인덱스 에러 완벽 방지)
    if len(man_data.shape) == 1:
        man_data = man_data.unsqueeze(1).repeat(1, p.IN_SEQ_LEN + p.TGT_SEQ_LEN)
        
    man_data_onehot = F.one_hot(man_data, num_classes=3) 

    # 2. 올바른 슬라이싱 (모델이 정답을 미리 보지 못하도록 IN_SEQ_LEN을 기준으로 1스텝 Shift)
    ...
    if p.MULTI_MODAL == True:
        ...
        batch_sweep = torch.arange(model.batch_size, device=device).view(-1, 1).expand(-1, p.TGT_SEQ_LEN)
        seq_sweep = torch.arange(p.TGT_SEQ_LEN, device=device).view(1, -1).expand(model.batch_size, -1)
    ...
    batch_print_info_dict = {
        'Total Loss': training_loss.item() / model.batch_size,
        'Traj Loss': traj_loss.item() / model.batch_size,
        'Man Loss': man_loss.item() / model.batch_size if p.MAN_DEC_OUT else 0,
        'Mode Partial Loss': mode_ploss.item() / model.batch_size if p.MAN_DEC_OUT else 0,
        'Man Partial Loss': man_ploss.item() / model.batch_size if p.MAN_DEC_OUT else 0,
        'Time Partial Loss': time_ploss.item() / model.batch_size if p.MAN_DEC_OUT else 0,
    }
...
def MTPM_loss(p, man_pred, man_vec_gt, n_mode, man_per_mode, device, test_phase = False, time_reg = True):
    ...
    man_loss = torch.mean(man_losses[batch_idx, winning_mode])
    time_loss = torch.mean(time_losses[batch_idx, winning_mode])
